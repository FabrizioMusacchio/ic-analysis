...
DEFAULT_DATASET_ROOT = PROJECT_ROOT / "example_data" / "synthetic_group_ab_place_learning"
...
DEFAULT_EXCLUDED_GROUPS: list[str] = []
...
DEFAULT_FIGSIZE_CM = {# always a pair of (width, height)
    ...
    "PHASE2_FIGSIZE_CM":        (15.0, 10.58), #(10.4, 7.0),
...
DEFAULT_SUMMARY_FIGSIZE_CM = (8.2, 7.1)
CM_TO_INCH = 2.54
# %% FUNCTIONS
...
def ordered_group_names(visits) -> list[str]:
    """Extract experimental-group names in their categorical display order."""
...
def render_mouse_age_at_phase1_start_plot(
    ...
    *,
    ...
    registered in the user-provided subject metadata.
    """
    ...
    age_metadata = metadata.drop(columns=["Phase1Start"], errors="ignore")
    age_table = age_metadata.merge(phase1_starts, on="RunGroup", how="left", validate="many_to_one").copy()
    ...
    age_table["age_years_at_phase1_start"] = age_table["age_days_at_phase1_start"] / 365.25
    unit_columns = {
        "days": ("age_days_at_phase1_start", "days"),
        "months": ("age_months_at_phase1_start", "months"),
        "years": ("age_years_at_phase1_start", "years")}
    if time_unit not in unit_columns:
        raise ValueError("time_unit must be one of 'days', 'months', or 'years'.")
    age_col, unit_label = unit_columns[time_unit]
    age_table = age_table.loc[age_table[age_col].notna()].copy()
    if age_table.empty:
        return
    ...
    save_table(age_table, output_root / f"mouse_age_at_phase1_start_{unit_label}_mouse.tsv")
    flagged = flag_iqr_outliers(
        age_table,
        value_col=age_col,
        group_cols=["Group"],
    )
    save_table(flagged, output_root / f"mouse_age_at_phase1_start_{unit_label}_mouse_with_outliers.tsv")
    omnibus, pairwise = compute_onset_group_statistics(
        flagged.rename(columns={age_col: "onset_hours"}),
        onset_col="onset_hours",
        phase_number=1,
        metric_name=f"mouse_age_at_phase1_start_{unit_label}",
        exclude_outliers=False,
    )
    save_table(omnibus, output_root / f"mouse_age_at_phase1_start_{unit_label}_omnibus_stats.tsv")
    save_table(pairwise, output_root / f"mouse_age_at_phase1_start_{unit_label}_pairwise_stats.tsv")
    plot_onset_violin(
        flagged.rename(columns={age_col: "onset_hours"}),
        onset_col="onset_hours",
        phase_display_name="Phase 1 start",
        title_label=title or "Mouse age at phase 1 start",
        ylabel=ylabel or f"Age [{unit_label}]",
        output_path=figure_output_path or output_root / f"mouse_age_at_phase1_start_{unit_label}_violin.png",
...
def _load_binned_result_table(output_root: Path, *, bin_hours: int, filename: str, dayphase: str = "all") -> pd.DataFrame:
    """Load one result table from a bin-specific CSV subfolder."""

    suffix = "" if dayphase == "all" else f"_{dayphase}"
    table_path = output_root / f"{int(bin_hours)}h{suffix}_bins" / "csv" / filename
    if not table_path.exists():
...
def _save_panel_figure(
    ...
    fig, ax = plt.subplots(
        1,
        1,
        figsize=(DEFAULT_SUMMARY_FIGSIZE_CM[0] / CM_TO_INCH, DEFAULT_SUMMARY_FIGSIZE_CM[1] / CM_TO_INCH),
    )
    draw_fn(ax)
...
def render_overview_plots(
    ...
    *,
    ...
    """Create full-experiment visit-activity plots for every experimental group."""
    ...
    plot_experiment_overview_groups(
        ...
        spread_metric=spread_metric,
        plot_style=plot_style,
        title_label=f"Group-average visits across all phases, mean ± {spread_metric.upper()}",
        origin_clock_hour=mouse_day_start_hour,
        awake_start_clock_hour=mouse_day_start_hour,
        awake_end_clock_hour=awake_end_clock_hour,
    )
...
def render_phase2_plots(
    ...
    *,
    ...
    """Create nose-poke adaptation summaries and paired-bar plots."""
    ...
    phase_tag = f"phase{int(phase_number)}"
    save_table(primary_mouse, output_dir / f"{phase_tag}_visits_mouse_bins_{bin_hours}h.tsv")
    save_table(primary_summary, output_dir / f"{phase_tag}_visits_group_summary_{bin_hours}h.tsv")
    save_table(secondary_mouse, output_dir / f"{phase_tag}_{secondary_metric}_mouse_bins_{bin_hours}h.tsv")
    save_table(secondary_summary, output_dir / f"{phase_tag}_{secondary_metric}_group_summary_{bin_hours}h.tsv")
    save_table(lick_positive_mouse, output_dir / f"{phase_tag}_lick_positive_visits_mouse_bins_{bin_hours}h.tsv")
    save_table(lick_positive_summary, output_dir / f"{phase_tag}_lick_positive_visits_group_summary_{bin_hours}h.tsv")
    save_table(lick_mouse, output_dir / f"{phase_tag}_lick_count_mouse_bins_{bin_hours}h.tsv")
    save_table(lick_summary, output_dir / f"{phase_tag}_lick_count_group_summary_{bin_hours}h.tsv")

    secondary_label = "Drinking visits" if secondary_metric == "lick_positive_visits" else "Lick count"
    phase2_end_hours = (
        visits.loc[visits["AnalysisPhaseNumber"].eq(int(phase_number))]
    ...
    for group_name in group_names:
        plot_phase2_adaptation(
            ...
            group_name=group_name,
            bin_hours=bin_hours,
            output_path=output_dir / f"{phase_tag}_visits_vs_{sanitize_filename_part(secondary_metric)}_{sanitize_filename_part(group_name)}_{bin_hours}h.png",
            secondary_label=secondary_label,
            phase_display_name=phase_display_names[int(phase_number)],
            plot_style=plot_style,
            spread_metric=spread_metric,
    ...
    common_phase2_end_hour = max(phase2_end_hours.values()) if phase2_end_hours else None
    plot_phase2_adaptation_groups(
        primary_summary,
        secondary_summary,
        group_names=group_names,
        bin_hours=bin_hours,
        output_path=output_dir / f"{phase_tag}_visits_vs_{sanitize_filename_part(secondary_metric)}_all_groups_{bin_hours}h.png",
        secondary_label=secondary_label,
        phase_display_name=phase_display_names[int(phase_number)],
        x_end_hours=common_phase2_end_hour,
        origin_clock_hour=phase_origin_hour,
        awake_start_clock_hour=mouse_day_start_hour,
        awake_end_clock_hour=awake_end_clock_hour,
        starting_day=phase_start_day,
...
def render_phase2_control_plots(
    ...
    *,
    ...
    plot_experiment_dual_metric_groups(
        ...
        group_names=group_names,
        bin_hours=bin_hours,
        output_path=output_dir / f"phase2_control_all_phases_visits_vs_drinking_visits_all_groups_{bin_hours}h.png",
        secondary_label="Drinking visits",
        phase_window_table=phase_window_table,
        phase_display_names=phase_display_names,
        origin_clock_hour=mouse_day_start_hour,
...
def render_phase_activity_plot(
    ...
    *,
    ...
    yticks: list[float] | tuple[float, ...] | None = None,
    title: str | None = None,
    ylabel: str | None = None,
) -> None:
    """Render the hourly activity boxplot across phases and groups."""
    ...
    save_table(mouse_phase_activity, output_dir / f"phase_activity_{metric_tag}_visits_per_hour_mouse.tsv")
    save_table(activity_stats, output_dir / f"phase_activity_{metric_tag}_visits_per_hour_stats.tsv")

    plot_phase_activity_boxplot(
        mouse_phase_activity,
        activity_stats,
        phase_display_names=phase_display_names,
        output_path=output_dir / f"phase_activity_{metric_tag}_visits_per_hour_boxplot.png",
        value_col=value_col,
        title=title or "Mice activity per group and phase",
        ylabel=ylabel or f"{metric_tag.capitalize()} number of corner visits per hour",
        figsize_cm=figsize_cm,
...
def render_cumulative_role_plots(
    ...
    *,
    ...
    mouse_day_start_hour: float,
    awake_end_clock_hour: float,
    selected_phase_numbers: list[int] | tuple[int, ...] | None = None,
    day_night_indicator: tuple[str, str] | None = ("awake", "sleep"),
    show_legend: bool = True,
    legend_loc: str | int | None = None,
    legend_font_size: float | None = None,
    ...
    available_phases = sorted(int(phase) for phase in visits["AnalysisPhaseNumber"].dropna().astype(int).unique())
    selected_phases = [int(phase) for phase in selected_phase_numbers] if selected_phase_numbers is not None else available_phases
    selected_phases = [phase for phase in selected_phases if phase in available_phases]
    if not selected_phases:
        return
    anchor_phase = 3 if 3 in selected_phases else selected_phases[0]
    mouse_counts, summary = compute_role_cumulative_curves(
        visits,
        pre_phase_hours=pre_phase_hours,
        anchor_phase_number=anchor_phase)
    ...
    phase_window_rows: list[dict[str, float | int]] = []
    anchor_start = float(scheduled_phase_start_hours[anchor_phase])
    for phase_number in selected_phases:
        phase_data = visits.loc[visits["AnalysisPhaseNumber"].eq(phase_number)].copy()
        if phase_data.empty:
            continue
        start_hours = float(phase_data["analysis_experiment_elapsed_hours"].min()) - anchor_start
        end_hours = float(phase_data["analysis_experiment_elapsed_hours"].max()) - anchor_start
        if phase_number < anchor_phase:
            start_hours = max(start_hours, -pre_phase_hours)
        phase_window_rows.append(
            {
                "PhaseNumber": phase_number,
                "start_hours": start_hours,
                "end_hours": end_hours})
    phase_window_table = pd.DataFrame(phase_window_rows)
    origin_clock_hour = phase_origin_clock_hour(mouse_day_start_hour, scheduled_phase_start_hours[anchor_phase])
    aligned_x_start = -pre_phase_hours + (mouse_day_start_hour - origin_clock_hour)
    cumulative_start_day = max(0, experiment_day_from_scheduled_start(scheduled_phase_start_hours[anchor_phase]) - 1)
    pl_onset = pd.DataFrame()
    pr_onset = pd.DataFrame()
    if 3 in selected_phases:
        _, _, pl_onset = compute_time_window_learning_curves(
            visits,
            phase_number=3,
            success_col="correct_corner_visit")
    if 4 in selected_phases:
        _, _, pr_onset = compute_time_window_learning_curves(
            visits,
            phase_number=4,
            success_col="correct_corner_visit")
    ...
    for group_name in group_names:
        ...
        pl_onset_group = (
            pl_onset.loc[pl_onset["Group"].astype(str).eq(group_name), "onset_hours"].dropna()
            if "Group" in pl_onset else pd.Series(dtype=float))
        pr_onset_group = (
            pr_onset.loc[pr_onset["Group"].astype(str).eq(group_name), "onset_hours"].dropna()
            if "Group" in pr_onset else pd.Series(dtype=float))
        if 3 in selected_phases and not pl_onset_group.empty:
            onset_points.append(
                {
                    "corner_role": "PL target corner",
                    "x_hours": float(scheduled_phase_start_hours[3] - anchor_start + pl_onset_group.median())})
        if 4 in selected_phases and not pr_onset_group.empty:
            onset_points.append(
                {
                    "corner_role": "PR target corner",
                    "x_hours": float(scheduled_phase_start_hours[4] - anchor_start + pr_onset_group.median())})
        plot_cumulative_role_curves(
            absolute_summary,
            group_name=group_name,
            ...
            plot_style=plot_style,
            phase_window_table=phase_window_table,
            phase_display_names={phase: phase_display_names[phase] for phase in selected_phases if phase in phase_display_names},
...
def run_analysis(
    *,
    ...
    cohort=None,
    aligned_visits: pd.DataFrame | None = None,
    ...
    """Run the synthetic Group A/B pipeline from a normal Python function call.

    This function mirrors the full cohort workflow but uses script-level settings
    so it can be imported and called without argument parsing.
    """
    ...
    selected_phase_display_names = DEFAULT_PHASE_DISPLAY_NAMES.copy()
    if phase_display_names:
        selected_phase_display_names.update({int(key): str(value) for key, value in phase_display_names.items()})
    output_root = Path(output_root) if output_root is not None else resolve_output_root(dataset_root, results_subdir)
    ...
    if cohort is None:
        ...
        cohort = load_cohort_data(
            ...
            group_names=list(selected_group_renames.keys()),
            subject_metadata=subject_metadata)
    else:
        print("Using cohort data already loaded by the experiment object...")
    ...
    for current_bin_hours in sorted(set(bin_hours)):
        ...
        render_overview_plots(
            ...
            group_names=group_names,
            phase_window_table=phase_window_table,
            phase_display_names=selected_phase_display_names,
            spread_metric=spread_metric,
            plot_style=plot_style,
            mouse_day_start_hour=mouse_day_start_hour,
            awake_end_clock_hour=awake_end_clock_hour,
