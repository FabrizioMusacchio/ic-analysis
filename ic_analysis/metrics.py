...
def filter_by_dayphase(
    ...
    *,
    ...
    time_col: str = "Start",
    elapsed_col: str = "analysis_experiment_elapsed_hours",
    origin_clock_hour: float = 6.0) -> pd.DataFrame:
    """Return rows from the active day phase, inactive night phase, or both."""

    if dayphase not in {"day", "night", "all"}:
        raise ValueError("`dayphase` must be one of 'day', 'night', or 'all'.")
    if dayphase == "all" or data.empty:
        return data.copy()
    if time_col in data.columns:
        ...
        clock_hour = (
            ...
            + timestamps.dt.microsecond / 3_600_000_000.0)
    elif elapsed_col in data.columns:
        clock_hour = (float(origin_clock_hour) + pd.to_numeric(data[elapsed_col], errors="coerce")) % 24.0
    else:
        raise ValueError(
            f"`data` must contain `{time_col}` or `{elapsed_col}` for dayphase filtering.")
    awake_start = float(awake_start_clock_hour) % 24.0
    awake_end = float(awake_end_clock_hour) % 24.0
    if awake_start == awake_end:
        is_day = pd.Series(True, index=data.index)
    elif awake_start < awake_end:
        is_day = clock_hour.ge(awake_start) & clock_hour.lt(awake_end)
    else:
        is_day = clock_hour.ge(awake_start) | clock_hour.lt(awake_end)
    keep = is_day if dayphase == "day" else ~is_day
    return data.loc[keep.fillna(False)].copy()
...
def compute_bottle_preference_bins(
    nosepokes: pd.DataFrame,
    *,
    phases: str | int | tuple[int, ...] | list[int] = "all",
    bin_h: int | float = 24,
    left_sides: tuple[int, ...] | list[int] = (1, 3, 5, 7),
    right_sides: tuple[int, ...] | list[int] = (2, 4, 6, 8),
    consumption_col: str = "LickNumber",
    calc: str = "right_bottle/left_bottle") -> tuple[pd.DataFrame, pd.DataFrame]:
    """Compute binned left/right bottle consumption or preference."""
    ...
    required_columns = {
        ...
        consumption_col}
    missing = sorted(required_columns.difference(nosepokes.columns))
    if missing:
        raise ValueError(f"Bottle-preference analysis requires missing column(s): {missing}")
    data = nosepokes.copy()
    data = data.loc[data["AnalysisPhaseNumber"].notna()].copy()
    data = data.loc[data["analysis_experiment_elapsed_hours"].notna()].copy()
    if phases != "all":
        selected_phases = [int(phases)] if isinstance(phases, int) else [int(phase) for phase in phases]
        data = data.loc[data["AnalysisPhaseNumber"].astype(int).isin(selected_phases)].copy()
    data = data.loc[data["Side"].notna()].copy()
    data["Side"] = pd.to_numeric(data["Side"], errors="coerce")
    data = data.loc[data["Side"].notna()].copy()
    data["bottle_side"] = pd.NA
    data.loc[data["Side"].astype(int).isin([int(side) for side in left_sides]), "bottle_side"] = "left"
    data.loc[data["Side"].astype(int).isin([int(side) for side in right_sides]), "bottle_side"] = "right"
    data = data.loc[data["bottle_side"].notna()].copy()
    if data.empty:
        empty_mouse, empty_summary = _empty_count_tables()
        for column in ["left_bottle_consumption", "right_bottle_consumption", "total_bottle_consumption"]:
            empty_mouse[column] = pd.Series(dtype=float)
        return empty_mouse, empty_summary
    data["_consumption"] = pd.to_numeric(data[consumption_col], errors="coerce").fillna(0.0).astype(float)
    data["bin_start_hours"] = np.floor(data["analysis_experiment_elapsed_hours"] / float(bin_h)) * float(bin_h)
    binned = (
        data.groupby(["Group", "ET", "ETLabel", "SEX", "bin_start_hours", "bottle_side"], observed=True)["_consumption"]
        .sum()
        .reset_index())
    pivot = (
        binned.pivot_table(
            index=["Group", "ET", "ETLabel", "SEX", "bin_start_hours"],
            columns="bottle_side",
            values="_consumption",
            aggfunc="sum",
            fill_value=0.0,
    ...
    mouse_bins = full_index.merge(
        ...
        how="left",
        validate="one_to_one")
    mouse_bins["left"] = mouse_bins["left"].fillna(0.0).astype(float)
    mouse_bins["right"] = mouse_bins["right"].fillna(0.0).astype(float)
    ...
    elif calc == "left_bottle/right_bottle":
        mouse_bins["value"] = mouse_bins["left_bottle_consumption"] / denominator
    elif calc == "right_bottle/left_bottle":
...
def compute_phase2_adaptation_bins(
    ...
    *,
    ...
    """Summarize nose-poke adaptation with two complementary metrics."""

    phase2 = visits.loc[visits["AnalysisPhaseNumber"].eq(int(phase_number))].copy()
    phase2["_value"] = 1.0
...
def compute_phase4_reversal_rate_bins(
    ...
    *,
    ...
    phase4 = visits.loc[visits["AnalysisPhaseNumber"].eq(int(phase_number))].copy()
    if not phase4.empty:
...
def _convert_success_rate_to_error_rate(
    mouse_table: pd.DataFrame,
    *,
    group_cols: list[str],
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Convert a success-rate table into an error-rate table."""

    if mouse_table.empty:
        return mouse_table.copy(), mouse_table.copy()
    error_table = mouse_table.copy()
    error_table["success_visits"] = error_table["correct_visits"].fillna(0.0)
    error_table["error_visits"] = (error_table["all_visits"].fillna(0.0) - error_table["success_visits"]).clip(lower=0.0)
    error_table["value"] = error_table["error_visits"] / error_table["all_visits"]
    error_table["value"] = error_table["value"].where(error_table["all_visits"].gt(0), np.nan)
    summary = (
        error_table.groupby(group_cols, observed=True)
        .agg(
            mean_value=("value", "mean"),
            median_value=("value", "median"),
            std_value=("value", "std"),
            mouse_n=("ET", "nunique"),
            contributing_mouse_n=("value", lambda values: int(values.notna().sum())),
            mean_success_visits=("success_visits", "mean"),
            mean_error_visits=("error_visits", "mean"),
            mean_all_visits=("all_visits", "mean"))
        .reset_index())
    summary["std_value"] = summary["std_value"].fillna(0.0)
    summary["sem_value"] = summary["std_value"] / np.sqrt(summary["mouse_n"].clip(lower=1))
    return error_table, summary

def compute_phase_segment_error_rate_tables(
    visits: pd.DataFrame,
    *,
    phase_number: int,
    success_col: str,
    ...
    """Compute mouse-level and group-level error rates for awake/sleep segments.

    The error rate is defined as ``(all_visits - success_visits) / all_visits``
    using the same denominator as :func:`compute_phase_segment_rate_tables`.
    """

    mouse_table, _ = compute_phase_segment_rate_tables(
        visits,
        phase_number=phase_number,
        success_col=success_col,
        origin_clock_hour=origin_clock_hour,
        ...
        awake_end_clock_hour=awake_end_clock_hour,
        max_days=max_days)
    return _convert_success_rate_to_error_rate(
        mouse_table,
        group_cols=["Group", "segment_order", "segment_day", "segment_name", "segment_label"])
...
def compute_awake_day_rate_tables(
    ...
    *,
    ...
    awake_end_clock_hour: float,
    max_days: int = 3,
    dayphase_segment: Literal["awake", "sleep", "all"] = "awake",
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Compute per-mouse day-wise rates for awake, sleep, or full-day segments."""
    ...
    mouse_table, summary = compute_phase_segment_rate_tables(
        ...
        awake_end_clock_hour=awake_end_clock_hour,
        max_days=max_days)
    if dayphase_segment == "all":
        mouse_table = (
            mouse_table.groupby(["Group", "ET", "ETLabel", "SEX", "segment_day"], observed=True)
            .agg(correct_visits=("correct_visits", "sum"), all_visits=("all_visits", "sum"))
            .reset_index())
        mouse_table["value"] = mouse_table["correct_visits"] / mouse_table["all_visits"]
        mouse_table["value"] = mouse_table["value"].where(mouse_table["all_visits"].gt(0), np.nan)
        mouse_table["segment_name"] = "all"
        mouse_table["segment_order"] = mouse_table["segment_day"].astype(int)
        mouse_table["segment_label"] = "Day " + mouse_table["segment_day"].astype(str) + " all"
        summary = (
            mouse_table.groupby(["Group", "segment_order", "segment_day", "segment_name", "segment_label"], observed=True)
            .agg(
                mean_value=("value", "mean"),
        ...
        mouse_table = mouse_table.loc[mouse_table["segment_name"].eq(dayphase_segment)].copy()
        summary = summary.loc[summary["segment_name"].eq(dayphase_segment)].copy()
...
def compute_awake_day_error_rate_tables(
    visits: pd.DataFrame,
    *,
    phase_number: int,
    success_col: str,
    ...
    """Compute per-mouse day-wise error rates for awake, sleep, or full-day segments.

    The error rate is ``(all_visits - success_visits) / all_visits``. With
    ``dayphase_segment="all"``, awake and sleep counts are summed before the
    rate is calculated, producing one full-day value per mouse and phase day.
    """
    ...
    mouse_table, _ = compute_awake_day_rate_tables(
        ...
        awake_end_clock_hour=awake_end_clock_hour,
        max_days=max_days,
        dayphase_segment=dayphase_segment)
    return _convert_success_rate_to_error_rate(
        mouse_table,
        group_cols=["Group", "phase_day", "segment", "segment_order", "segment_label"])
...
def compute_awake_day_ratio_tables(
    ...
    *,
    ...
    """Compute daily ratios from two visit-level indicator columns.

    Ratios are computed per mouse as ``numerator_count / denominator_count``.
    ``dayphase_segment`` selects awake, sleep, or full-day windows. With
    ``"all"``, awake and sleep counts are summed before the ratio is
    calculated.
    """
    ...
    phase_visits = phase_visits.loc[phase_visits["segment_day"].between(1, max_days)].copy()
    if dayphase_segment == "all":
        phase_visits["segment_name"] = "all"
        phase_visits["segment_order"] = phase_visits["segment_day"].astype(int)
        phase_visits["segment_label"] = "Day " + phase_visits["segment_day"].astype(str) + " all"
    else:
        phase_visits = phase_visits.loc[phase_visits["segment_name"].eq(dayphase_segment)].copy()
    if phase_visits.empty:
        return _empty_count_tables()
    ...
    day_frame = pd.DataFrame(
        [
            {
                "segment_day": day,
                "segment_name": dayphase_segment,
                "segment_order": day if dayphase_segment == "all" else (day - 1) * 2 + (1 if dayphase_segment == "awake" else 2),
                "segment_label": f"Day {day} {dayphase_segment}",
            }
            for day in range(1, max_days + 1)
        ])
    mice["__key"] = 1
...
def compute_role_cumulative_curves(
    ...
    *,
    ...
    anchor_start = (
        visits.loc[visits["AnalysisPhaseNumber"].eq(int(anchor_phase_number)), "analysis_phase_start_hours"].dropna().min()
    )
    if pd.isna(anchor_start):
        return pd.DataFrame(), pd.DataFrame()

    phase_visits = visits.copy()
    phase_visits["combined_phase_elapsed_hours"] = phase_visits["analysis_experiment_elapsed_hours"] - float(anchor_start)
    if phase_visits.empty:
        return pd.DataFrame(), pd.DataFrame()
...
def compute_phase_activity_medians(
    ...
    *,
    ...
    """Compute per-mouse hourly visit activity summaries for each phase.

    Missing hourly bins are filled with zeros so the output reflects activity
    over the full available phase window. Sparse data can therefore have a
    median of zero; ``mean_visits_per_hour`` is usually the more informative
    ...
    mouse_activity = (
        ...
        .agg(
            median_visits_per_hour="median",
            mean_visits_per_hour="mean",
            active_hours=("count"),
            total_visits="sum")
        .reset_index()
        .rename(columns={"AnalysisPhaseNumber": "PhaseNumber"}))
    return mouse_activity.sort_values(["Group", "PhaseNumber", "ET"]).reset_index(drop=True)
...
def compute_phase_activity_statistics(mouse_phase_activity: pd.DataFrame) -> pd.DataFrame:
    ...
    value_col = "mean_visits_per_hour" if "mean_visits_per_hour" in mouse_phase_activity.columns else "median_visits_per_hour"
    rows: list[dict[str, float | str | int]] = []
    for group_name, group_data in mouse_phase_activity.groupby("Group", observed=True):
        phase_values = {
            phase_number: group_data.loc[group_data["PhaseNumber"].eq(phase_number), value_col].to_numpy()
        ...
        if len(group_data["PhaseNumber"].unique()) >= 2:
            tukey = pairwise_tukeyhsd(
                endog=group_data[value_col].to_numpy(),
