...
This module is responsible for turning the raw IntelliCage text exports into a
single harmonized analysis table. It reads:

1. `Mice.txt` for mouse-level metadata and assigned reward corners.
2. `Visits.txt` for visit-level events and the visit-level lick summary.
3. `Nosepokes.txt` for nose-poke events that can be linked back to visits by
   `(RunGroup, Phase, VisitID)`.

The loader adds analysis-friendly columns such as phase numbers, relative
experiment time, relative phase time, and strict rewarded-visit flags. The
strict rewarded-visit definition follows the current poster goal and therefore
requires:

- correct place visit (`PlaceError == 0`)
- at least one associated nose-poke event
- at least one lick during the visit

The original MATLAB-compatible metric (`PlaceError == 0` only) is preserved in
its own boolean column so both definitions remain available for comparison.
"""
...
class CohortData:
    ...
    ----------
    visits:
        Visit-level analysis table after metadata merge and nose-poke
        aggregation.
    metadata:
        Mouse-level metadata compiled from all `Mice.txt` files.
    nosepokes:
        Raw nose-poke events across all groups and phases.
    phase_manifest:
        Phase-level manifest that stores start and end timestamps as well as
        visit counts for each run group and phase.
    """
...
def _ordered_group_categories(groups: pd.Series) -> list[str]:
    ...
    ----------
    groups:
        Series containing the pathology labels stored in the original `VIRUS`
        column and renamed to `Group`.

    Returns
    -------
    list[str]
        Preferred groups first, followed by any unexpected labels in
        alphabetical order so that the workflow remains robust to future
        cohorts.
    """
...
def read_mice_metadata(mice_path: Path, run_group: str) -> pd.DataFrame:
    """Read one `Mice.txt` file and normalize the metadata columns.

    Parameters
    ----------
    mice_path:
        Path to the tab-separated `Mice.txt` file inside one IntelliCage run
        group.
    run_group:
        Name of the run-group directory, for example `Gruppe1`.

    Returns
    -------
    pandas.DataFrame
        Mouse metadata table with harmonized column names and helper columns
        such as `ETLabel`.
    """
...
def read_visits_file(visits_path: Path, run_group: str, phase_name: str) -> pd.DataFrame:
    ...
    ----------
    visits_path:
        Path to a phase-specific `Visits.txt` file.
    run_group:
        Name of the run-group directory, for example `Gruppe1`.
    phase_name:
        Phase label in the folder structure, for example `Phase3`.

    Returns
    -------
    pandas.DataFrame
        Visit-level table with parsed timestamps and explicit run/phase
        identifiers.
    """
...
def read_nosepokes_file(nosepokes_path: Path, run_group: str, phase_name: str) -> pd.DataFrame:
    ...
    ----------
    nosepokes_path:
        Path to a phase-specific `Nosepokes.txt` file.
    run_group:
        Name of the run-group directory, for example `Gruppe1`.
    phase_name:
        Phase label in the folder structure, for example `Phase3`.
    ...
    -------
    pandas.DataFrame
        Nose-poke event table with parsed timestamps and explicit run/phase
        identifiers.
    """
...
def summarize_nosepokes_by_visit(nosepokes: pd.DataFrame) -> pd.DataFrame:
    """Aggregate nose-poke events to the visit level.

    Parameters
    ----------
    nosepokes:
        Raw nose-poke table returned by :func:`read_nosepokes_file`.

    Returns
    -------
    pandas.DataFrame
        One row per `(RunGroup, Phase, VisitID)` with counts and sums that are
        helpful for later metrics. The summary intentionally keeps the raw visit
        table as the primary source while exposing nose-poke-derived helper
        columns.
    """
...
def _build_phase_manifest(visits: pd.DataFrame) -> pd.DataFrame:
    """Create a manifest that stores the temporal extent of each phase.

    Parameters
    ----------
    visits:
        Visit-level table before or after metadata merge.

    Returns
    -------
    pandas.DataFrame
        One row per run group and phase with phase start/end timestamps and
        visit counts.
    """
...
def _attach_time_reference_columns(visits: pd.DataFrame, phase_manifest: pd.DataFrame) -> pd.DataFrame:
    ...
    ----------
    visits:
        Visit-level table after metadata and nose-poke aggregation.
    phase_manifest:
        Manifest created by :func:`_build_phase_manifest`.

    Returns
    -------
    pandas.DataFrame
        Copy of the visit table with reference timestamps and elapsed-time
        helper columns.
    """
    ...
    enriched["experiment_day"] = np.floor(enriched["experiment_elapsed_hours"] / 24.0).astype(int) + 1
...
def load_cohort_data(dataset_root: Path | str) -> CohortData:
    """Load and harmonize an IntelliCage place-learning cohort directory.

    Parameters
    ----------
    dataset_root:
        Root directory of one cohort, for example
        `Data IntelliCage/BioMedX_4MonthCohort_2019`.

    Returns
    -------
    CohortData
        Dataclass containing the merged visit table, mouse metadata, raw
        nose-poke events, and a phase manifest.

    Raises
    ------
    FileNotFoundError
        Raised if the expected `Mice.txt`, `Visits.txt`, or `Nosepokes.txt`
        files are missing.
    ValueError
        Raised if visit rows cannot be matched back to mouse metadata.
    """
